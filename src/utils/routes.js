...
//accessToken文件主要依赖js-cookie模块，把accessToken，setToken，removeToken设置在这里
import { getAccessToken } from "@/utils/accessToken";
//get-page-title文件主要是网站的标题，get-page-title通过依赖setting.config.js里的title变量修改网站标题
//如果需要更改网站的标题，可以直接到setting.config.js修改title属性
import getPageTitle from "@/utils/getPageTitle";
const { loginInterception } = require("../config");
...
router.beforeEach(async (to, from, next) => {
  ...
  // 这里的accessToken()就是在上面导入的accessToken.js里的accessToken()方法
  const hasToken = getAccessToken();

  //如果存在token，即存在已登陆的令牌
  if (hasToken || !loginInterception) {
